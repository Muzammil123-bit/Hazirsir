...
import {
  ImageBackground,
  StyleSheet, View, Text, BackHandler, TouchableOpacity, Image, Modal } from 'react-native';
...
class ChatScreen extends Component {
  ...
  async componentDidMount() {
    ...
    await this.setState({orderId:ord, posterId:prof, uid:value, chatName:name})
  ...
  async onSendImage(){
    ...
    const url='https://www.hazirsir.com/web_service/add_chat.php';//url||||||||||||||||||||||||||||||||||||||||||||||||||
    ...
    return new Promise(async function(resolve, reject) {
      ...
      .then((responseJson) => {
        resolve(responseJson);
          // Showing response message coming from server after inserting records.
          // resolve(responseJson);
          // console.warn("qqqwwwwwqw"+ JSON.stringify(responseJson));
        //   console.log(responseJson.data)
          if(responseJson.data==='"Record created successfully" '){
            this.sendToServer()
          }
  ...
  async sendToServer(){
    ...
    const url='https://www.hazirsir.com/web_service/read_chat.php';//url||||||||||||||||||||||||||||||||||||||||||||||||||
    await fetch(url, {
        method: 'POST',
        headers: {
  ...
  renderImage = () => {
    return (
      <TouchableOpacity>
        <Image source={require('../Logos/play.png')} />
      </TouchableOpacity>
    );
    return null;
  }
    ...
    return (
      <View style={{flex:1, opacity:this.state.photoSelect?0.4:1}}>
        <View style={{width:'100%', height:55,backgroundColor:'#f5f5f5' ,justifyContent:'center', paddingLeft:12}}>
          <Text style={{fontSize:17, fontWeight:'bold'}}>{this.state.chatName}</Text>
        </View>
        <GiftedChat
