...
let funcRep = null;
class GigChat extends Component {
    constructor(props) {
        super(props);
        this.state = {
            orderId: '',
    ...
    async componentDidMount() {
        ...
        var Apply_gig_id = this.props.navigation.getParam('Apply_gig_id', '0');
        var Recever_id = this.props.navigation.getParam('Recever_id', '0');
        var ChatName = this.props.navigation.getParam('ChatName', ' ');
        var Gig_title = this.props.navigation.getParam('Gig_title', ' ');
        var Gig_requirement = this.props.navigation.getParam('Gig_requirement', '0');
        var Gig_ans = this.props.navigation.getParam('Gig_ans', '0');
        ...
        await this.setState({
            ...
            Apply_gig_id: Apply_gig_id,
            taskTitle: ChatName,
            skillName: Gig_title,
            dueDate: Gig_requirement,
            taskDesc: Gig_ans,
    ...
    async onSendImage2() {
        ...
        await uploaddata.push({ 'name': 'apply_gig_id', 'data': this.state.Apply_gig_id });
        if (this.state.start) {
        ...
        const url = 'https://www.hazirsir.com/web_service/upload_task_image_gig.php';//url||||||||||||||||||||||||||||||||||||||||||||||||||
    ...
    async onSendImage() {
        ...
        await uploaddata.push({ 'name': 'receiver_id', 'data': this.state.Receiver_id });
        ...
        const url = 'https://www.hazirsir.com/web_service/add_chat_gig.php';//url||||||||||||||||||||||||||||||||||||||||||||||||||
        return new Promise(async function (resolve, reject) {
            await RNFetchBlob.fetch(
                "POST",
                url,
    ...
    async sendToServer() {
        ...
        const url = 'https://www.hazirsir.com/web_service/read_chat_gig.php';//url||||||||||||||||||||||||||||||||||||||||||||||||||
        await fetch(url, {
            method: 'POST',
            headers: {
            ...
            body: JSON.stringify({
                z: [this.state.uid, this.state.Receiver_id, this.state.Apply_gig_id],
            })
        })
            .then((response) => response.json())
    ...
    async getWorkImages() {
        console.log('workImg')
        const url = 'https://www.hazirsir.com/web_service/read_task_image_gig.php';//url||||||||||||||||||||||||||||||||||||||||||||||||||
        await fetch(url, {
            method: 'POST',
            headers: {
                Accept: 'application/json',
            ...
            body: JSON.stringify({
                z: [this.state.uid, this.state.Apply_gig_id],
            })
        })
            .then((response) => response.json())
            .then(async (responseJson) => {
        ...
        return (
            <View style={{ flex: 1, opacity: this.state.photoSelect ? 0.4 : 1 }}>
                <View style={{ paddingVertical: 5, width: '100%' }}>
                    <View style={{ width: '100%', flexDirection: 'row', alignItems: 'center', paddingLeft: 12 }}>
                        <Text style={{ fontSize: 13, fontFamily: 'Montserrat-Bold' }}>Name:{' '}</Text>
                        <Text style={{ fontSize: 12 }}>{this.state.taskTitle}</Text>
                    </View>
                    <View style={{ width: '100%', flexDirection: 'row', alignItems: 'center', paddingLeft: 12 }}>
                        <Text style={{ fontSize: 13, fontFamily: 'Montserrat-Bold' }}>Title:{' '}</Text>
                    ...
                    <View style={{ width: '100%', flexDirection: 'row', alignItems: 'center', paddingLeft: 12, justifyContent: 'space-between' }}>
                        <View style={{ flexDirection: 'row' }}>
                            <Text style={{ fontSize: 13, fontFamily: 'Montserrat-Bold' }}>Requirement:{' '}</Text>
          ...
          <View> */}
                ...
                <Modal
                    ...
                    <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center' }}>
                        <Text>{'Requirement Answer: ' + this.state.taskDesc}</Text>
                    </View>
                </Modal>
            </View>
        );
