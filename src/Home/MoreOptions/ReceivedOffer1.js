...
class ReceivedOffer1 extends Component {
    constructor(props) {
        super(props);
        this.state = {
            uid: '',
    ...
    async sendToServer() {
        ...
        await fetch(url, {
            ...
            .then(async (responseJson) => {
                await this.setState({ data: responseJson[0] })
                // if(responseJson[0].length)
                // {
                //     alert("if");
                // }
                // else
    ...
    async accept_reject(gig_id, status, pkg_id) {
        const url = 'https://www.hazirsir.com/web_service/accept_gig_request.php';
        await fetch(url, {
            method: 'POST',
            headers: {
                Accept: 'application/json',
            ...
            body: JSON.stringify({
                z: [this.state.uid, gig_id, status, pkg_id],
            })
        })
            .then((response) => response.json())
            .then(async (responseJson) => {
                                            ...
                                            <View style={{ backgroundColor: '#ffffff', marginTop: 4, marginBottom: 4, marginHorizontal: 10 }}>
                                                ...
                                                >
                                                    ...
                                                    <View style={{ flexDirection: "row" }}>
                                                        ...
                                                        <View >
                                                            ...
                                                            <View style={{ flexDirection: "row" }}>
                                                                ...
                                                                <Text style={{ marginLeft: 10, fontSize: 13, marginTop: 4 }}>{item.package_detail}</Text>
                                                            </View>
                                                            <View style={{ flexDirection: "row" }}>
                                                    ...
                                                    <View style={{ flexDirection: "row", alignSelf: "flex-end", }}>
                                                        <TouchableOpacity
                                                            onPress={() => this.accept_reject(item.id, "Reject", item.package_id)}
                                                        ...
                                                        <TouchableOpacity
                                                            onPress={() => this.accept_reject(item.id, "Accept", item.package_id)}
...
export default withNavigation(ReceivedOffer1);
